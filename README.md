...
- Figures (png + pdf) → `artifacts/plots/`
  ...
  12. `plot11_latency_cdf_common_lambda` — iso-load counterpart to plot3:
     plain / naive / proposed@`seg2_batch` all replayed at ONE shared λ
     ...
     structure with the different offered loads; this one removes the load term
     so the remaining gap is attributable to batching alone (mainly proposed's
