...
- Figures (png + pdf) → `artifacts/plots/`
  1. `plot1a_slo_goodput_vs_plain` / `plot1b_slo_goodput_vs_naive` /
     `plot1c_slo_goodput_vs_proposed` — SLO vs goodput, three identical figures:
     BOTH baselines (plain, naive) plus the proposed `seg2_batch` sweep in each,
     all replayed on the same arrival trace at that figure's λ. They differ ONLY
     in whose capacity sets the operating point — 1a plain's, 1b naive's, 1c
     proposed's — so read as a series they answer "who still meets which SLO as
     the load is raised to each design's ceiling in turn". 1c is the extreme
     end: at proposed's capacity both baselines are past divergence, so their
     goodput collapses across the whole SLO range. λ is AUTO-derived by default
     (D_anchor − step, the same rule as e2e Table B);
     `plots.slo_goodput_lambda.{plain,naive,proposed}` overrides manually
     (0 = saturated). Since `proposed` is a b2 sweep, 1c must pick one variant
     to read a capacity off: `plots.slo_goodput_b2` = `"max_capacity"`
     (default — the b2 sustaining the highest load, i.e. the true right edge of
     the design space), `"config"` (`batching.seg2_batch`), or a specific b2.
     Every b2's measured capacity is printed; state the chosen one in the
     caption.
  ...
  11. `plot1d_peak_goodput_bars` — peak goodput per runtime, each at its OWN
